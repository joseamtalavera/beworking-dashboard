...
const Booking = ({ mode = 'user', userProfile, initialView }) => {
  ...
  const handleCloseCreateDialog = useCallback(() => {
    ...
    setSlotBookingRoom(null);
    setSlotBookingTime(null);
  }, []);

  // Re-clicking the "Espacios" sidebar entry while inside the deep booking
  // flow should bring you back to the floor-plan root, just like the Volver
  // arrow does. Volver remains active.
  useEffect(() => {
    const onNavClick = (e) => {
      if (e?.detail?.tabId === 'Booking') {
        handleCloseCreateDialog();
      }
    };
    window.addEventListener('dashboard:nav-click', onNavClick);
    return () => window.removeEventListener('dashboard:nav-click', onNavClick);
  }, [handleCloseCreateDialog]);
